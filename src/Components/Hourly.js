    ...
    return (
        <div className="hourly__weather">
            <div className="container">
                <div className="forecast__card animate__animated animate__fadeInDown">
                <p className="cards__name">Hourly Weather in {`${weather?.location?.name}, ${weather?.location?.country}`}</p>
                    {weather?.forecast?.forecastday[0].hour.map((el)=>(
                        <div className="forecast__grid">
                        <p className="forecast__info">{el.time}</p>
